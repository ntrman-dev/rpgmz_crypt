#!/usr/bin/env python3
"""
RPG Maker Data JSON Encrypt/Decrypt Tool

Decrypts or encrypts RPG Maker data JSON files using the same algorithm
...
Usage:
    python rpgdata_crypt.py decrypt <input_dir> <output_dir> [--pretty]
    python rpgdata_crypt.py encrypt <input_dir> <output_dir>
    python rpgdata_crypt.py decrypt-file <input.json> <output.json> [--pretty]
    python rpgdata_crypt.py encrypt-file <input.json> <output.json>
    python rpgdata_crypt.py restore <game_dir>
    python rpgdata_crypt.py revert <game_dir>
    python rpgdata_crypt.py patch-js <game_dir>
"""
...
def decrypt_file(input_path: str, output_path: str, params: CryptoParams,
    ...
    if "data" not in wrapper:
        raise ValueError(
            f"{input_path}: not an encrypted RPG Maker data JSON file "
...
def patch_mv_managers_js(js_path: str) -> bool:
    ...
    marker = "if(c.bid){var b=Buffer.from(c.data,'base64');"
    if marker in content:
        print("  JS already patched (plain JSON support detected).")
        return False

    old_loader = "var c=JSON.parse(xhr.responseText);var b=Buffer.from(c.data,'base64');"
    new_loader = "var c=JSON.parse(xhr.responseText);if(c.bid){var b=Buffer.from(c.data,'base64');"
...
def cmd_restore(game_dir: str) -> None:
    ...
    print()
    print("To undo, run:  python rpgdata_crypt.py revert " + game_dir)
...
def main():
    parser = argparse.ArgumentParser(
        description="RPG Maker data JSON encrypt/decrypt tool"
    )
    ...
    dec = subparsers.add_parser(
        ...
        description="Decrypt encrypted RPG Maker data JSON files to plain JSON.",
    )
    ...
    enc = subparsers.add_parser(
        ...
        description="Encrypt plain JSON files back to the wrapped RPG Maker data JSON format.",
    )
    ...
    dec_f = subparsers.add_parser(
        ...
        description="Decrypt a single RPG Maker data JSON file.",
    )
    ...
    enc_f = subparsers.add_parser(
        ...
        description="Encrypt a single JSON file back to the wrapped RPG Maker data JSON format.",
    )
    ...
    restore = subparsers.add_parser(
        ...
        description=(
            "Decrypt all data/*.json files in place and patch the detected "
            "manager script so the engine can read plain JSON directly. "
            "Creates backups (data.encrypted/ and a manager-script .bak) for safe undo.\n\n"
    ...
    restore.add_argument(
        "game_dir",
        help="Root directory of the supported RPG Maker game (contains data/ and js/)",
    )
    ...
    revert = subparsers.add_parser(
        ...
        description="Restore data/ and the detected manager script from backups.",
    )
    revert.add_argument(
        "game_dir",
        help="Root directory of the supported RPG Maker game (contains data/ and js/)",
    ...
    pjs = subparsers.add_parser(
        ...
        description=(
            "Modify the detected manager script so it accepts both encrypted and plain "
